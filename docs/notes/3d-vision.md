...
获取了相机的参数之后，我们要想完成三维重建，还需要获取两张照片中的对应点，也就是我们需要找出那些点描述的是同一个物体。一个朴素的方法是对两张图片进行特征点提取，然后进行逐一匹配，这个肯定是可以的，但效率上有待改进。

我们来考虑为什么上面我们的朴素方法效率不好。

参考：[计算机视觉算法中的 相机标定（Camera Calibration）](https://bbs.huaweicloud.com/blogs/411042)
