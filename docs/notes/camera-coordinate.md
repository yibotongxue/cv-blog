...
首先合并从世界坐标系的坐标到相机坐标系的坐标之间的平移变换和旋转变换，将矩阵合并为 $\left[
  \begin{matrix}
  R & T \\
  0 & 1
  \end{matrix}
...
然后我们将除了除了投影矩阵前面的三个矩阵合并以下，记为 $\left[
  \begin{matrix}
  \frac{f}{dx} & 0 & c_x \\
  0 & \frac{f}{dy} & c_y \\
  0 & 0 & 1
...
在前面我们已经指出， $c_x$ 和 $c_y$ 指的是图像坐标系的原点（也就是主光轴与光屏的交点）在相片上对应的像素的像素坐标。一般情况下主光轴与光屏的交点都是位于感光区域的中心，也就是其在相片上对应的像素也是相片的中心，从而一般地， $c_$ 和 $c_y$ 为相片长和宽的一半。
