...
随后我们需要将图像坐标系上像的坐标对应到像素坐标系上。我们之前提到了这两个坐标系一个描述的是像的位置信息，一个描述的是记录了像的信息的照片上像素的位置信息，一个像素和像上一个区域对应，这样的区域大小是固定的。考虑到像素坐标系原点对应的像上的区域与图像坐标系的原点并不重合（一般情况下，图像坐标系的坐标原点，也就是主光轴与光屏的交点都是在感光区域的中心，对应的是相片中心的像素，不过有些时候不是这样），如果我们直接将图像坐标系映射到照片上，得到的实际上是一个坐标原点建立在图像坐标系原点对应的像素上，坐标轴方向与像素坐标系的一致（这里需要注意光屏上成的像是倒立的）的中间坐标系。假设一个像素在 $x$ 轴对应实际的长度为 $dx$ ，那么实现上面的映射过程**只需将图像坐标系下的坐标除以 $dx$ 即可得到该点在照片上对应的像素中间坐标系下的 $x$ 轴坐标， $y$ 轴类似**。这在数值上是一个缩放变换，可以通过对图像坐标系下的坐标作用一个放缩矩阵 $\left[
  \begin{matrix}
  \frac{1}{dx} & 0 \\
  0 & \frac{1}{dy}
  \end{matrix}
\right]$ 。随后我们需要求得像素坐标系下的坐标，这其实就是一个坐标系变换求坐标变换的问题，根据之前的分析可以很容易的得到需要对坐标进行一个平移变换，平移变换的向量为中间坐标系原点在像素坐标系下的坐标，记为 $\left[
  \begin{matrix}
  c_x \\
  c_y
  \end{matrix}
...
:::warning
你应该阅读完这篇[知乎回答](https://www.zhihu.com/question/36296104/answer/83199096)，了解了齐次坐标下变换如何用矩阵表示之后，再继续阅读这篇笔记。
:::

[知乎回答](https://www.zhihu.com/question/36296104/answer/83199096)中我们特别需要了解的就是平移变换的矩阵和旋转变换的矩阵，平移矩阵可以写作 $\left[
  \begin{matrix}
  I & T \\
  0 & 1
  \end{matrix}
\right]$ ，其中 $T$ 为 $n$ 维的向量，对应到 $n$ 维空间的变换，实现的是将一个 $n$ 维的向量 $a$ 映射成 $a + T$ 也就是 $T$ 是 $n$ 维空间的时候平移对应的平移向量。而旋转矩阵可以写作 $\left[
  \begin{matrix}
  R & 0 \\
  0 & 1
  \end{matrix}
\right]$ ，其中 $R$ 是 $n$ 维的旋转矩阵，实现的是将 $n$ 维空间的向量 $a$ 映射成 $Ra$ ，也就是 $R$ 是 $n$ 维空间的时候旋转变换对应的矩阵。
...
完成到图像坐标系的映射后，我们再考虑到像素坐标系的映射，这只需要完成一步缩放变换和一步平移变换，这都是比较容易的，这里我们直接给出相应的矩阵 $\left[
  \begin{matrix}
  \frac{1}{dx} & 0 & 0 \\
  0 & \frac{1}{dy} & 0 \\
  0 & 0 & 1
...
\right]$ 和 $\left[
  ...
  0 & 0 & 1
  \end{matrix}
  \right]$。但是，最后我们还需要记的将齐次坐标转换回二维的像素坐标，这需要除以最后一个维度的坐标，也就是 $z$ 。

到这里，我们就在引入齐次坐标的情况下重新完成了坐标变换的推导。重新回顾一下过程中我们提到的坐标变换，**首先我们将世界坐标系下的坐标变为齐次坐标（也就是一个四维向量），随后我们在对齐次坐标作用平移变换和旋转变换，而后我们截去最后一个维度，再进行放缩，得到图像坐标系下的像的齐次坐标（也就是一个三维向量），最后我们对这个齐次坐标进行平移变换和放缩，然后从得到的齐次坐标获得二维的像素坐标**。如果我们有一个世界坐标系下的点的坐标为 $\left[
  \begin{matrix}
  x \\
  y \\
  z
  \end{matrix}
\right]$ ，设其对应的像素在像素坐标系下坐标为 $\left[
  \begin{matrix}
  x' \\
  y'
  \end{matrix}
...
在实际的计算中，我们还会合并一些变换的矩阵。我们通过引入齐次坐标的方式使得坐标变换都可以用矩阵乘法表示，而矩阵乘法具有结合律，我们利用这点可以合并一些变换。

我们将除了除了投影矩阵前面的三个矩阵合并以下，记为 $\left[
  \begin{matrix}
  \frac{f}{dx} & 0 & c_x \\
  0 & \frac{f}{dy} & c_y \\
  0 & 0 & 1
...
有内参矩阵，自然也有外参矩阵，一般成为从世界坐标系到相机坐标系的坐标变换的矩阵为外参矩阵，也就是 $\left[
  \begin{matrix}
  R & 0 \\
  0 & 1
  \end{matrix}
...
我们上面的推导已经给出了摄像过程中的坐标变换的基本过程，其中涉及很多参数，在实际计算的过程中我们需要知道它们的含义，同时知道它们通常情况下的取值。

#### 平移向量 $T$ 的含义和取值

$T$ 表示的是世界坐标系下的坐标到相机坐标系下需要加上的平移向量，这对应的是一个平移变换。根据上面提到的[知乎文章](https://zhuanlan.zhihu.com/p/661060377)，这个平移变换是世界坐标系到相机坐标系的平移变换的逆变换。不难得到，从世界坐标系到相机坐标系的坐标系平移变换向量正好为相机的坐标，所以 $T$ 为相机坐标的相反向量。

#### 旋转矩阵 $R$ 的含义和取值

跟上面分析一样， $R$ 是世界坐标系到相机坐标系的坐标系的旋转矩阵的逆矩阵，具体取值需要根据具体情况确定。

#### $d_x$ 和 $d_y$ 的含义和取值

在前面我们就已经指出， $d_x$ 和 $d_y$ 指的是相片上一个像素的对应**像**上的区域在图像坐标系 $x$ 轴和 $y$ 轴的的大小。一般地它们是相等的。

#### $c_x$ 和 $c_y$ 的含义和取值

在前面我们已经指出， $c_x$ 和 $c_y$ 指的是图像坐标系的原点（也就是主光轴与光屏的交点）在相片上对应的像素的像素坐标。一般情况下主光轴与光屏的交点都是位于感光区域的中心，也就是其在相片上对应的像素也是相片的中心，从而一般地， $c_x$ 和 $c_y$ 为相片长和宽的一半。
